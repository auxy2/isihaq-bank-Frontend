  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setError(
          res.data.message || 'Failed to top up wallet. Please try again.'
        );
      }
    } catch (err) {
    ...
    if (accountNumber) {
      try {
        const res = await axios.post(
          'https://abank.vercel.app/api/account/verify',
          {
            accountNumber,
            bankCode,
          }
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://abank.vercel.app/api/transfer',
        {
          username,
          accountNumber,
          accountName,
      ...
      {isTopUpFormOpen && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>Top Up Wallet</h3>
            <button onClick={toggleTopUpForm} className="close-button">
              X
            </button>
            ...
            <button onClick={handleTopUp} className="submit-button">
              Submit
            </button>
          </div>
        </div>
